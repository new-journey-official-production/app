...
import { Wallet, Truck, Plus } from "lucide-react";
        ...
        <div className="space-y-8">
          <section className="rounded-2xl border border-border p-6" data-testid="checkout-address-section">
            ...
            {addresses.length > 0 && !showForm && (
              <RadioGroup value={addressId} onValueChange={setAddressId} className="space-y-2">
                {addresses.map((a) => (
                  <label key={a.id} htmlFor={String(a.id)} className={`flex gap-3 rounded-xl border p-4 cursor-pointer ${addressId === a.id ? "border-zinc-950 dark:border-white" : "border-border"}`} data-testid={`address-${a.id}`}>
          ...
          <section className="rounded-2xl border border-border p-6" data-testid="checkout-payment-section">
            ...
            <RadioGroup value={method} onValueChange={setMethod} className="grid sm:grid-cols-2 gap-3">
              <PaymentTile id="upi" label="Pay with UPI" icon={Wallet} checked={method === "upi"} />
            ...
            {method === "upi" && (
              <div className="mt-3 text-xs text-muted-foreground rounded-md bg-muted p-3">
                After placing the order you will be taken to a secure payment page with an amount-specific UPI QR that works with all UPI apps.
              </div>
            )}
          </section>
