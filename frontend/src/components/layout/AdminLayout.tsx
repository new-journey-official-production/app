...
export default function AdminLayout() {
  ...
  const [openSections, setOpenSections] = useState<Record<string, boolean>>({
    orders: true, catalog: true, b2b: true, analytics: false, operations: false, content: false, system: false,
  });
  ...
  return (
    <div className="min-h-screen grid grid-cols-1 lg:grid-cols-[240px_1fr] bg-zinc-50 dark:bg-zinc-950">
      <aside className="hidden lg:flex flex-col border-r border-border bg-white dark:bg-zinc-900" data-testid="admin-sidebar">
        <SidebarHeader />
        <AdminNav openSections={openSections} b2bOpen={b2bOpen} onToggleSection={toggleSection} onB2bOpenChange={setB2bOpen} onNavigate={() => {}} />
  ...
  return (
    <nav className="flex-1 overflow-y-auto p-3 space-y-0.5">
      ...
      <Collapsible open={b2bOpen} onOpenChange={onB2bOpenChange} className="pt-1">
        ...
        <CollapsibleContent className="space-y-0.5 mt-0.5 pl-1">
          {B2B_LINKS.map((l) => <NavItem key={l.to} {...l} nested onNavigate={onNavigate} />)}
        </CollapsibleContent>
      </Collapsible>
    </nav>
  );
