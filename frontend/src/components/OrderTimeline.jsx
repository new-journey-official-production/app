      ...
      <div className="space-y-4">
        {cancelled ? (
          <div className="relative pl-10">
            ...
            <div className="text-xs text-muted-foreground">{byKey.cancelled?.at?.slice(0, 16).replace("T", " ")}</div>
          </div>
        ) : (
          ORDER_STATUS_STEPS.map((step, i) => {
            ...
            return (
              <div key={step.key} className="relative pl-10">
                ...
                {entry?.at && <div className="text-xs text-muted-foreground font-mono-data">{entry.at.slice(0, 16).replace("T", " ")}</div>}
