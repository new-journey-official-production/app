  ...
  return (
    <div className="p-6 lg:p-8">
      ...
      <div className="grid gap-6">
        <InvSection title="Filaments" items={filaments} onDel={del} />
        <InvSection title="Other supplies" items={other} onDel={del} />
      </div>
    </div>
  );
}

function InvSection({ title, items, onDel }) {
  return (
    <section>
      <div className="font-display font-semibold mb-3">{title}</div>
      <div className="rounded-xl border border-border bg-card overflow-hidden">
        <Table>
          <TableHeader><TableRow><TableHead>Name</TableHead><TableHead>Material</TableHead><TableHead>Color</TableHead><TableHead>Quantity</TableHead><TableHead>Reorder</TableHead><TableHead>Unit cost</TableHead><TableHead></TableHead></TableRow></TableHeader>
          <TableBody>
            {items.map((i) => (
              <TableRow key={i.id}>
                ...
                <TableCell><Button variant="ghost" size="icon" className="h-8 w-8 text-red-600" onClick={() => onDel(i.id)}><Trash2 className="h-4 w-4" /></Button></TableCell>
              </TableRow>
            ))}
          </TableBody>
        </Table>
